...
if (!([string]::IsNullOrWhiteSpace(($Global:KeldorConfig).CMLibrary))) {
...
if (!([string]::IsNullOrWhiteSpace(($Global:KeldorConfig).EAC))) {
...
if (!([string]::IsNullOrWhiteSpace(($Global:KeldorConfig).HomeAssistant))) {
...
if (!([string]::IsNullOrWhiteSpace(($Global:KeldorConfig).iLO))) {
...
if (!([string]::IsNullOrWhiteSpace(($Global:KeldorConfig).Remedy))) {
...
if (!([string]::IsNullOrWhiteSpace(($Global:KeldorConfig).SDNMgmt))) {
...
if (!([string]::IsNullOrWhiteSpace(($Global:KeldorConfig).vCenter))) {
...
if (Test-Path "$env:ProgramFiles\Microsoft VS Code\Code.exe") {
    ...
    if (!([string]::IsNullOrWhiteSpace(($Global:KeldorConfig).VSCodeSettingsPath))) {
    ...
    if (!([string]::IsNullOrWhiteSpace(($Global:KeldorConfig).VSCodeExtRepo))) {
...
# Keldor menu displayed in the Context menu
$MenuWSTools = New-Object System.Windows.Forms.MenuItem
$MenuWSTools.Text = "Keldor"
...
#Stop app services
if (!([string]::IsNullOrWhiteSpace(($Global:KeldorConfig).AppNames))) {
...
#Update Visio
if ((Test-Path ([System.Environment]::GetFolderPath("MyDocuments") + "\My Shapes")) -and (!([string]::IsNullOrWhiteSpace(($Global:KeldorConfig).Stencils)))) {
...
$Menu_Restart_Tool.add_Click({
        Start-Process powershell.exe -ArgumentList "`$host.ui.RawUI.WindowTitle = 'Keldor Taskbar App'; D:\OneDrive\Projects\Scripting\NeedToWorkOn\WSTools_GUI.ps1"
