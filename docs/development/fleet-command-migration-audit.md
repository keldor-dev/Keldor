...
| `Get-ComputerModel` | Windows | Low-risk improvement | `ComputerName[]`; pipeline binding present | Structured inventory object | No documented stable type name | Preserve shape and add a reviewed type name | Medium | P2 |
...
| `Get-SerialNumber` | Windows | Compatibility-sensitive | Optional `ComputerName[]`; no pipeline binding | Object; failure uses the string `NA` in `SerialNumber` | Failure is encoded in a data property; no diagnostics/type name | Stable inventory result with `IsSuccessful` and error fields; preserve legacy property | Medium | P2 |
| `Get-UpTime` | Windows | Requires redesign | Optional `ComputerName[]`; no pipeline binding | Success has date/numeric fields plus formatted `Total`; failure changes all types to strings | Embedded units and inconsistent success/failure types | Per-target health/inventory result using `[timespan] Duration` and structured errors | High | P1 |
| `Test-Online` | Windows | Compatibility-sensitive | Optional `ComputerName[]`; no pipeline binding | Object with target property `Name` and string `Status` | Noncanonical identity; no Boolean availability or diagnostics | Add `ComputerName` and nullable `IsAvailable`; preserve `Name` and current states | High | P1 |
